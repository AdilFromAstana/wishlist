...
Веб-приложение «Список желаний» для двух пользователей. Общий wishlist виден
обоим, каждый item принадлежит конкретному пользователю.

**Стек:** Next.js (App Router) · React · TypeScript · Tailwind CSS · Supabase
(PostgreSQL + Auth + Storage) · деплой на Vercel. Валюта — KZT (₸).

## Возможности

- Вход через **magic link** (email OTP), сессия сохраняется (`persistSession`,
  `autoRefreshToken`, `detectSessionInUrl`).
- Автоматическое восстановление сессии при открытии приложения.
- Защита маршрутов: неавторизованный пользователь видит только Login.
- CRUD для wishlist items + загрузка фото в Supabase Storage.
- Фильтры (пользователь, цена min/max, приоритет, статус, активность, дата) и
  сортировка (дата, цена, приоритет).
- Страница пользователей и личный кабинет со статистикой.
...
3. Откройте **SQL Editor** и выполните весь файл
   ...
   - таблицы `profiles` и `wishlist_items`;
